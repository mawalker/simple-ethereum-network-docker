...
#    sleepTime = 30 #should be fine with our default template network-config.json file.
...
#    newFilterID = makeNewFilter(ip=sys.argv[1],port=sys.argv[2],fromBlock="0x0",verbose='False')
#    print ("New Filter ID: " + str(newFilterID))


    # Submit the contract as a transaction.
#    transactionReceipt = deployContract(ip=sys.argv[1],port=sys.argv[2],contractBytecode=contractBytecode,verbose='False')
...
#    contractAddress = getAddressOfTransaction(ip=sys.argv[1],port=sys.argv[2],transactionReceipt='0x28488615f0a5f6a7a3631ed7bbdb37ec14c8f052d94fc0beb4ac4d711b326722',verbose='True')
#    pprint.pprint(contractAddress)
#    print ("Contract Address: " + contractAddress['contractAddress'])

    # Call 'test()' in the smart contract.
#    transactionReceipt2 = callContractMethod(ip=sys.argv[1],port=sys.argv[2],toAddress=contractAddress,dataString="0xf8a8fd6d",gas="0x200000",account=None,verbose='True')
...
#    time.sleep(sleepTime)
 ...
 #   changeResults = getFilterChanges(ip=sys.argv[1],port=sys.argv[2],filterID=newFilterID,verbose="True")
